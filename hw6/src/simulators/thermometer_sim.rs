//! UDP-имитатор умного термометра.
//!
//! Читает INI-подобный конфиг:
...
// Выделено в отдельную функцию чтобы тесты не касались файловой системы.
...
// `keep_going` принимает замыкание для управляемой остановки в тестах.
...
// Простой LCG-генератор — без внешних зависимостей.
...
mod tests {
    ...
    fn pseudo_random_temp_not_constant() {
        ...
        assert!(unique.len() > 1, "Генератор выдаёт одно и то же значение");
    }
    ...
    fn sim_packets_are_valid_temperatures() {
        ...
        for _ in 0..5 {
            ...
            let val: f64 = s
                .trim()
                .parse()
                .unwrap_or_else(|_| panic!("Пакет '{}' не является f64", s));
            assert!(
