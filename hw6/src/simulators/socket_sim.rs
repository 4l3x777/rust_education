//! Неблокирующий TCP-имитатор умной розетки.
//!
//! Все клиенты обслуживаются в одном потоке через опрос (poll-loop).
//! `LineBuffer` накапливает байты до `\n`, не блокируя цикл.
//!
//! Запуск: `cargo run --bin socket_sim -- 127.0.0.1:7878`
//!
//! Протокол: `ON` / `OFF` → `OK`, `STATE` → `ON`|`OFF`, `POWER` → ватты.
...
impl LineBuffer {
    /// Возвращает:
    /// - `Ok(Some(line))` — полная строка готова
    /// - `Ok(None)` — клиент закрыл соединение
    /// - `Err(WouldBlock)` — данных пока нет
...
pub fn run_socket_simulator(addr: &str) {
    ...
    println!("[Имитатор розетки] Слушаю на {} (неблокирующий режим)", addr);
    ...
    loop {
        ...
        // Без паузы цикл будет жечь процессор вхолостую
