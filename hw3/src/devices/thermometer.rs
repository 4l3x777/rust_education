        ...
        let handle = thread::spawn(move || {
            ...
            loop {
                ...
                match socket.recv_from(&mut buf) {
                    Ok((n, _)) => {
                        if let Ok(s) = std::str::from_utf8(buf.as_slice().get(..n).unwrap_or(&[])) {
...
mod tests {
    ...
    fn udp_thermometer_no_data_returns_error() {
        let t = SmartThermometer::udp("Тест", "127.0.0.1:0")
            .expect("бинд должен удасться");
    ...
    fn udp_thermometer_receives_temperature() {
        ...
        let therm = SmartThermometer::udp("Тест", "127.0.0.1:0")
            .expect("бинд");
        ...
        let therm2 = SmartThermometer::udp("Тест̦", &bind_addr.to_string())
